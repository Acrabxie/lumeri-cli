...
const videoPkg = JSON.parse(readFileSync(new URL("../packages/lumeri-video/package.json", import.meta.url), "utf8"));
const quantaPkg = JSON.parse(readFileSync(new URL("../packages/lumeri-quanta/package.json", import.meta.url), "utf8"));
const compatPkg = JSON.parse(readFileSync(new URL("../packages/lumeri-cli/package.json", import.meta.url), "utf8"));
assert.equal(pkg.name, "lumeri-cli-source");
assert.equal(pkg.private, true);
assert.equal(pkg.version, "1.0.1");
assert.equal(pkg.license, "MIT");
assert.match(readFileSync(new URL("../LICENSE", import.meta.url), "utf8"), /Copyright \(c\) 2026 Acrabxie/);
assert.equal("bin" in pkg, false, "the private source root must not expose an installable CLI");
assert.equal("publishConfig" in pkg, false, "the private source root must not be publishable");
assert.deepEqual(videoPkg.bin, { luvi: "bin/luvi.js" });
assert.deepEqual(quantaPkg.bin, { luqu: "bin/luqu.js" });
assert.deepEqual(compatPkg.bin, { luvi: "bin/luvi.js", luqu: "bin/luqu.js" });
for (const productPkg of [videoPkg, quantaPkg, compatPkg]) {
  assert.deepEqual(productPkg.publishConfig, { access: "public", registry: "https://registry.npmjs.org/" });
  assert.equal("lumeri" in productPkg.bin, false);
}
for (const field of ["repository", "homepage", "bugs"]) assert.equal(field in pkg, false);
for (const script of ["mock", "deploy-preview"]) assert.equal(script in pkg.scripts, false);
...
assert.doesNotMatch(videoHelp.stdout, /\b(?:login|logout|whoami|account|model|codex)\b/i);
...
assert.doesNotMatch(quantaHelp.stdout, /\b(?:login|logout|whoami|account|model|codex)\b/i);
...
assert.equal(seen.some((item) => /^\/(?:auth|accounts|model)(?:\/|$)/.test(item.url)), false);
