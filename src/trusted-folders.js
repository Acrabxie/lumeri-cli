...
export function canonicalFolderPath(folder) {
  const requested = resolve(String(folder || ""));
  let target;
  try {
    target = realpathSync.native(requested);
  } catch (cause) {
    const error = new Error(`folder must be an existing directory: ${requested}`, { cause });
    error.code = "E_FOLDER_NOT_FOUND";
    throw error;
  }
  if (!statSync(target).isDirectory()) {
    const error = new Error(`folder is not a directory: ${requested}`);
    error.code = "E_FOLDER_NOT_DIRECTORY";
    throw error;
  }
  return target;
}

export function resolveTrustedFolder(folder, options = {}) {
  let target;
  try { target = canonicalFolderPath(folder); }
  catch { return null; }
  return getTrustedFolders(options).includes(target) ? target : null;
}

export function isTrustedFolder(folder, options = {}) {
  return resolveTrustedFolder(folder, options) !== null;
}

export function trustFolder(folder, { home = homedir() } = {}) {
  const target = canonicalFolderPath(folder);
