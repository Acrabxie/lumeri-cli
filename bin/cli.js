...
const OPTION_TOKENS = new Set([
  "-h", "--help", "-V", "--version", "-p", "--prompt", "-s", "--server",
  "--json", "--output-schema", "--no-splash", "--no-preview", "--no-browser",
]);

function isOptionToken(value) {
  return OPTION_TOKENS.has(value)
...
function requireOptionValue(argv, index, option) {
  ...
  if (value === undefined || isOptionToken(value)) {
    process.stderr.write(`${commandName}: ${option} requires a value\n`);
    process.exit(2);
  }
  return value;
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "-h" || a === "--help") opts.help = true;
      ...
      opts.prompt = requireOptionValue(argv, i, a);
      i += 1;
    }
    else if (a.startsWith("--prompt=")) opts.prompt = a.slice("--prompt=".length);
    else if (a === "--json") opts.json = true;
    else if (a === "--output-schema") {
      opts.outputSchema = requireOptionValue(argv, i, a);
      i += 1;
    }
    else if (a.startsWith("--output-schema=")) opts.outputSchema = a.slice("--output-schema=".length);
    else if (a === "-s" || a === "--server") {
      opts.server = requireOptionValue(argv, i, a);
      i += 1;
    }
...
Options
  -p, --prompt <text> Run one non-interactive Lumeri Agent turn
      --json            Print session events as JSONL (requires --prompt)
      --output-schema <file>
                        Validate the final JSON response against a JSON Schema
...
if ((opts.json || opts.outputSchema != null) && opts.prompt == null) {
  process.stderr.write(`${commandName}: --json and --output-schema require -p/--prompt\n`);
  process.exit(2);
}
if (opts.outputSchema === "") {
  process.stderr.write(`${commandName}: --output-schema requires a file path\n`);
  process.exit(2);
}
